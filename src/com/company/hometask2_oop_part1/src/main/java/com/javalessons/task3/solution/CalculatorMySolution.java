...
public class CalculatorMySolution {
    ...
    public CalculatorMySolution(final int loanAmount, final int numberOfYears, double interestRatePerYear) {
    }

    public CalculatorMySolution(final int loanAmount, final int numberOfYears, double interestRatePerYear, int extraMonthlyPayment) {
    ...
    void calculateAndPrint(){
        double payment = calcMonthPayment(loanAmount, monthlyInterestRate);
        double monthStartBalance = loanAmount;
        double interestAmount = 0.0d;
        int periodCount = 0;
        double monthEndBalance = monthStartBalance;
        System.out.println("MONTH, STARTING_BALANCE, PAYMENT, INTEREST, PRINCIPAL, ENDING_BALANCE, TOTAL_INTEREST");
        ...
        while (periodCount < years * MONTHS) {
            ...
            if (monthEndBalance >= payment) {
                ...
                System.out.printf("%d, %15.2f, %12.2f, %8.2f, %15,2f, %15.2f%n", periodCount, monthStartBalance, payment +extraMonthlyPayment,
                        monthlyInterest, monthlyPrincipal, monthEndBalance, interestAmount);
                monthStartBalance = monthEndBalance;
                if ((monthlyPrincipal + extraMonthlyPayment) > monthEndBalance){
                    extraMonthlyPayment = 0;
                }
            } else {
                System.out.printf("%d, %15.2f, %12.2f, %8.2f, %15.2f, %15.2f%n", periodCount, monthStartBalance, monthEndBalance,
                        0.0, monthEndBalance, 0.0, interestAmount);
                break;
            }
        }
    }
     ...
     * P = ( Pv * R) / (1 -(1 + R)
     */
    double calcMonthPayment(int loanAmount, double monthlyInterestRate){
